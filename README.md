...
For this workshop you will use a free trial of Astro to run Airflow and the Astro IDE to write dags.
...
4. Run `git checkout airflow-3-ide` to switch to the airflow 3 branch.
...
8. To enable scheduled dag runs in your new Airflow Deployment, click on the drop down next to `Sync to test`, and click `Test Deployment Details`. Go to the `Environment` tab, click `Edit Deployment Variables`, and delete the `AIRFLOW__SCHEDULER__USE_JOB_SCHEDULE` variable.
8. Go back to the Astro IDE, and in the drop down next to `Sync to Test`, click on `Open Airflow`.
